...
mod tests {
    ...
    fn point_is_valid() {
        ...
        assert!(Point::new(std::f64::consts::FRAC_PI_2, std::f64::consts::PI).is_valid()); // Edge cases
        assert!(Point::new(-std::f64::consts::FRAC_PI_2, -std::f64::consts::PI).is_valid()); // Edge cases

        assert!(!Point::new(std::f64::consts::FRAC_PI_2 + 0.1, 0.0).is_valid()); // Invalid lat
        assert!(!Point::new(-std::f64::consts::FRAC_PI_2 - 0.1, 0.0).is_valid()); // Invalid lat
        assert!(!Point::new(0.0, std::f64::consts::PI + 0.1).is_valid()); // Invalid lon
        assert!(!Point::new(0.0, -std::f64::consts::PI - 0.1).is_valid()); // Invalid lon
    }
}
